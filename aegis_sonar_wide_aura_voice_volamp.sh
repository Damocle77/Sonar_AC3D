...
# │     - DSP invariato                                                          │
# │     - Filtergraph assemblato a blocchi nominati                              │
# │     - Pipeline più leggibile: input -> split -> voice -> surround -> output  │
# │     - Stessa firma sonora, meno giungla mentale                              │
# ╰──────────────────────────────────────────────────────────────────────────────╯
...
build_filter_complex() {
  local input_graph output_graph
  input_graph="$(build_input_split_graph)"
  output_graph="$(build_output_join_graph)"
  printf '%s\n%s\n%s\n%s\n' "$input_graph" "$VOICE_BLOCK" "$SUR_BLOCK" "$output_graph"
}
