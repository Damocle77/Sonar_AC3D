...
set -uo pipefail
# ╭──────────────────────────────────────────────────────────────────────────────╮
# │   audio_analyzer.sh - V4.1 VOLAMP - Marzo 2026                               │
# │                                                                              │
...
# │   processore aegis_sonar_wide_aura_voice.sh + volamp per-file.               │
# │                                                                              │
...
PARAMETRI:
  ...
  modo    : probe  (Default) Solo struttura tracce audio.
            lra    Misura LRA (EBU R128), suggerisce preset e volamp.
            delta  Misura Delta SUR-FC, suggerisce preset e volamp.
...
VOLAMP HEURISTIC:
  ...
  usando la Loudness Integrata del file intero. E' una euristica prudente,
  non una verita' scolpita nella pietra: serve a intercettare tracce nate
  un po' basse senza massacrare il limiter.
...
for f in "${FILES[@]}"; do
  case "$f" in
    ...
    *_EAC3_Aegis.mkv|*_EAC3_Sonar.mkv|*_EAC3_Wide.mkv|*_EAC3_Aura.mkv|*_EAC3_Voice.mkv)
      info "Skip output già processato: $f"
      continue
      ;;
    *)
      FILTERED_FILES+=("$f")
...
(( ${#FILES[@]} == 0 )) && { err "Nessun file valido trovato da processare."; exit 1; }
...
measure_stream_loudness() {
  ...
  local i_val
  i_val=$(grep -E "^\s+I:\s+-?[0-9]" "$log_file" | tr -d '
'           | awk '{print $2}' | tail -1)
  rm -f "$log_file"
...
# Target prudente: evita di pompare tutto, ma intercetta file nati bassi.
# ────────────────────────────────────────────────────────────────────────────────
loudness_to_volamp() {
  local i_val="$1"
...
scan_lra() {
  ...
  ffmpeg -y -nostdin -hide_banner -nostats     -i "$f"     -map "0:${target_stream}"     -af "ebur128=framelog=verbose"     -vn -sn -f null -     >/dev/null 2>"$log_file" </dev/null || true
...
'           | awk '{print $2}' | tail -1)
  ...
  local lra_val
  lra_val=$(grep -E "^\s+LRA:\s+-?[0-9]" "$log_file" | tr -d '
'             | awk '{print $2}' | tail -1)

  local peak_val
  peak_val=$(grep -E "^\s+Peak:\s+-?[0-9]" "$log_file" | tr -d '
'              | awk '{print $2}' | tail -1)
  ...
  echo -e "  [1;33mLoudness Integrata:[0m  ${i_val:-N/A} LUFS"
  echo -e "  [1;33mLRA (Dinamica):    [0m  ${p_color}${lra_val} LU[0m"
  echo -e "  [1;33mTrue Peak:         [0m  ${peak_val:-N/A} dBTP"
  echo -e "  [1;37mPreset Consigliato:[0m  ${p_color}${preset}[0m  (${preset_desc})"
  echo -e "  [1;37mVolume sorgente:   [0m  [1;36m${volume_status}[0m"
  echo -e "  [1;37mVolamp:            [0m  [1;36m${volamp} dB[0m  (${volamp_desc})"

  if [[ "$max_ch" =~ ^[0-9]+$ && "$max_ch" -ne 6 ]]; then
    echo -e "  [0;33m⚠  Preset non applicabile: il processore richiede 5.1 (6 canali)[0m"
  fi
  echo ""
}
...
if [[ "$MODE" == "lra" || "$MODE" == "delta" ]] && [[ "${#GLOBAL_METRIC_VALUES[@]}" -gt 1 ]]; then
  ...
  if [[ "$MODE" == "delta" ]]; then
    ...
    case "$season_preset" in
      AURA)  season_desc="Surround piuttosto scarsi nella maggioranza episodi"  ;;
    ...
    case "$season_preset" in
      VOICE) season_desc="Contenuto fortemente compresso (broadcast, lossy)"   ;;
      AURA)  season_desc="Serie TV standard, dinamica ridotta"                 ;;
      WIDE)  season_desc="Blu-ray serie TV, film moderni"                      ;;
...

  {
    ...
    echo "# ── Batch generato da audio_analyzer (V4.1 VOLAMP - ${MODE^^}) ──"
